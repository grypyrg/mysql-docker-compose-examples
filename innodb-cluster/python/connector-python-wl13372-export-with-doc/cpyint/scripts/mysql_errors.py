...
class MySQLErrorsProcessor(object):
    ...
    def _init_server_errors(self):
        ...
        err_server_file = ERR_SERVER_FILE if self._mysql_version < (8, 0, 0) \
            else os.path.join("share", "errmsg-utf8.txt")
    ...
    def _init_client_errors(self):
        ...
        err_client_cfile = ERR_CLIENT_CFILE if self._mysql_version < (8, 0, 0) \
            else os.path.join("libmysql", "errmsg.cc")
    ...
    def _parse_server_errors(self):
        ...
        for line in fp:
            ...
            elif line.startswith(('OBSOLETE_ER_', 'ER_', 'WARN_')):
        ...
        with codecs.open(self._client_errmsg_cfile, 'r', 'latin1') as fp:
            ...
            for line in fp:
                ...
                if line.startswith('"') and line.endswith('"'):
                    ...
                    message.append(line.replace('"', '').rstrip(','))
                    err_name, err = self.get_error_by_number(nr)
                    self._errors[err_name]['messages'][lang] = "".join(message)
                    self._lang_count[lang] = self._lang_count.get(lang, 0) + 1
                    del message[:]
                    nr += 1
    ...
    def _serverr_string(self, line):
        ...
        # Make sure we got only valid characters
        matches = RE_ERRNAME.match(res[0])
        if not matches:
            raise RuntimeError("Failed getting error name; line was '%s'" % line)
        res[0] = matches.group(0)
        return res
...
def main():
    ...
    try:
        ...
        err_server_file = ERR_SERVER_FILE if mysql_version < (8, 0, 0) \
            else os.path.join("share", "errmsg-utf8.txt")
        err_client_cfile = ERR_CLIENT_CFILE if mysql_version < (8, 0, 0) \
            else os.path.join("libmysql", "errmsg.cc")
        checks = [
